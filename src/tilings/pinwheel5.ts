/*import { Triangle, Vec2, tileViewport } from "./Tiles";
const colorizer = (s:number, l:number) => (t:Triangle) =>
  "hsl(" +
  (((Math.round(
    3 * (t.a.x - t.b.x) +
      7 * (t.a.y - t.b.y) +
...
const fromSSide = (l:Vec2) =>
  Triangle(Vec2(0, 0), l.perp(), l.scale(2).add(l.perp()));
...
export default (ctx:CanvasRenderingContext2D) =>
  tileViewport(
    ...
    () =>
      fromSSide(Vec2(Math.random() * 200, Math.random() * 200)).translate(
        Vec2(
          Math.random() * ctx.canvas.width * 4 + ctx.canvas.width / 2,
